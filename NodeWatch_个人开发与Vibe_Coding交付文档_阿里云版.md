...
- 正式公网访问使用受信任的公网 IP HTTPS 证书；HTTP 只用于 ACME 验证并自动跳转到 HTTPS。
...
- 减少一套容器和反向代理配置。
- 前后端同域，不需要额外处理 CORS。
- 登录 Cookie、静态页面和 API 使用同一个公网入口，理解成本更低。
...
服务器基线：Ubuntu 24.04、2 vCPU、2 GiB 内存、40 GiB 系统盘。公网 IP、SSH 私钥和数据库密码均不得写入仓库。
...
- 当前项目不配置域名，使用受信任的公网 IP HTTPS 证书；如果未来改用中国内地域名，应另行核对并完成当时适用的备案要求。
...
7. 创建宿主机 Nginx HTTP 验证模板和公网 IP HTTPS 模板。
...
11. 不直接替我登录阿里云、保存 SSH 私钥、邮箱或生产凭据。
...
- 公网 IP 启用受信任 HTTPS 后，HTTP 自动跳转 HTTPS，证书模拟续期通过。
...
此阶段仅用于确认链路和 ACME 验证，不算最终生产完成。HTTP 会明文传输 Cookie 和 Agent Token，因此不要在 HTTP 环境登录或运行真实 Agent。

## 78. 公网 IP HTTPS 与自动续期

当前项目不配置域名，使用 Let’s Encrypt 的受信任公网 IP 证书。公网 IP 证书有效期约 6 天，必须配置自动续期；Certbot 必须为 5.4 或更高版本。

1. 使用 Snap 安装新版 Certbot：
...
```text
/etc/letsencrypt/live/nodewatch-ip/fullchain.pem
/etc/letsencrypt/live/nodewatch-ip/privkey.pem
```

4. 使用 `deploy/nginx-nodewatch-ip-https.conf.example` 替换临时 HTTP 配置，执行 `sudo nginx -t` 后重载 Nginx。最终配置必须保留 `/.well-known/acme-challenge/`，将其他 HTTP 请求跳转到 HTTPS，并从 HTTPS 反向代理到 `127.0.0.1:8000`。
...
- 模拟续期成功，`snap.certbot.renew.timer` 存在。

HTTPS 验收后首次登录，立即修改 bootstrap 管理员密码，从服务器 `.env` 删除 `BOOTSTRAP_ADMIN_USERNAME` 和 `BOOTSTRAP_ADMIN_PASSWORD`，再重建 App。域名与 ICP 备案不属于当前项目交付范围；将来若改用中国内地域名，应另行核对并完成当时适用的备案要求。
...
7. 公网 IP 是否仍绑定当前服务器。
8. Certbot IP 证书是否有效并包含当前公网 IP。
9. Certbot 证书路径和 Nginx 配置是否一致。
10. 不要同时改端口、证书和启动命令；一次只排查一个变量。
...
- Nginx、公网 IP HTTPS、ACME 验证和自动续期。
